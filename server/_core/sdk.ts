...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Check if this is a local user (format: "local:123") or OAuth user
    ...
    // Update last signed in for all users
    // Update last signed in for OAuth users only (local users don't have openId)
    if (user.openId) {
      await db.upsertUser({
        openId: user.openId,
        lastSignedIn: signedInAt,
